...
class Order extends BaseComponent {
    ...
    async orderGoodsPreview (req, res, next) {
        ...
        let goodsList = []
        let goodsInfo = await GoodsModel.findOne({id: params.goodsId}, '-_id')
    ...
    async orderCartPreview (req, res, next) {
        ...
        for (let item of cartData.goodsList) {
            let goodsInfo = await GoodsModel.findOne({id: item.goodsId}, '-_id')
    ...
    async userOrderList (req, res, next) {
        let { page = 0, size = 20 } = req.body
        let list = await OrderModel.find({}, '-_id').skip(Number(page*size)).limit(Number(size))
        // console.log(list)
